...
> **Note:** $\delta_{SNR}$ and $\tau_{SNR}$ are domain-specific instantiations of $R = \delta/\tau$ applied to SNR degradation monitoring. $R_{em} \neq \text{SNR}$. R is a structural ratio (deviation relative to absorption thickness) and is unrelated to SNR (signal-to-noise ratio) as an information-theoretic concept.
