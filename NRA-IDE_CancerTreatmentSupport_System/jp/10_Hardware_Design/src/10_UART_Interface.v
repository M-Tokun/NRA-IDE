    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin rx_state <= 0; rx_valid <= 0; rx_cnt <= 0; end
            ...
            case (rx_state)
                ...
                1: if (rx_cnt == BIT_PERIOD/2) begin rx_state <= 2; rx_cnt <= 0; rx_bit_idx <= 0; end 
                   else rx_cnt <= rx_cnt + 1;
                2: if (rx_cnt == BIT_PERIOD) begin
                       rx_cnt <= 0; rx_shift[rx_bit_idx] <= uart_rx;
                       if (rx_bit_idx == 7) rx_state <= 3; else rx_bit_idx <= rx_bit_idx + 1;
                   end else rx_cnt <= rx_cnt + 1;
                3: if (rx_cnt == BIT_PERIOD) begin rx_state <= 0; rx_data <= rx_shift; rx_valid <= 1; end 
                   else rx_cnt <= rx_cnt + 1;
            endcase
        end
    end
