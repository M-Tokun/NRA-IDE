...
| `deform_velocity` | $v$ | μm/s | 0.0 - 255.0 | `0x0000` - `0xFF00` | 変形速度。標準値 200.0 (`0x3200`) |
...
* `0x02` は旧版で「負の物理量」と定義されていたが、Q8.8 は符号なしであり、ホスト側 `_float_to_q8_8()` が負値を 0 にクランプするため**発生不能なコードだった**。「範囲外」に再定義することで、クランプ結果（0）を含めて検出可能になる。
