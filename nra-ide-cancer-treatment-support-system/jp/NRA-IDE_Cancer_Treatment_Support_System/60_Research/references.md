...
> **NRA-IDEへの適用:** Type Aモデルにおいて、低い硬度（0.5 kPa付近）でもPASSABLE判定を出すロジックの裏付け。
...
> **NRA-IDEへの適用:** `pore_size` の範囲（5.0-15.0 μm）設定の根拠。毛細血管内皮の間隙は通常5-10 μm、腫瘍血管では最大15 μm程度まで拡大する。文献の報告値（3-8 μm）に対し、システムの下限は 5.0 μm とやや保守的に設定してある（Phase 4 §1、60_Research §4「Normal Capillary: 5-8 μm」参照）。
