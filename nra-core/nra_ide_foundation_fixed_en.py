# FILE: nra_ide_foundation_fixed_en.py
...
# Date: 2026-03-11 05:50 JST

# [FIX LOG]
#  - PLOT Path: Synchronized filename with JP version "nra_foundation_plot_2026-02-20_2355.png".
#  - BoundLaw.correct(): Fixed E_stored update logic (from overwrite to differential accumulation).
#  - ObservationLog.record(): Expanded violation detection to all targets in the cycle.
...
# ─────────────────────────────────────────────
# DynamicState (Normalized Container)
# ─────────────────────────────────────────────
...
@dataclass
class EnergyLedger:
    E_in       : float = 0.0   # Input Energy
    E_stored   : float = 0.0   # Stored Energy
    E_dissipated: float = 0.0  # Dissipated Energy
...
if __name__ == "__main__":
    ...
    # Fixed PLOT path to relative for CI compatibility and synchronized with JP version
