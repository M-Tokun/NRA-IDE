...
# ==============================================================================
# ⚠️ 安全境界の明示 (Safety Boundary Declaration)
# ------------------------------------------------------------------------------
# 本ツール (nra_crystallizer) は「人間向けテキスト要約支援」ツールであり、
# NRA-IDEの安全ゲート本体ではない。
#
# SINGULARITY_KEYWORDS による意味的キーワードマッチングは、テキスト結晶化の
# 実用上の機能として許容されるが、NRA-IDE安全ゲート（コヒーレンスゲート、
# 因果ダイオード等）の実装にこのロジックを流用することは厳禁である。
#
# 理由: キーワードの「意味」に依存した判定は、NRA-IDEの
#        「意味を扱わない（Non-Semantic）」原則に抵触する。
#
# また、momentum は閾値調整用の内部変数であり、NRA-IDEの τ（厚み）
# そのものではない。τ は物理的に常に正の値である。
# ==============================================================================
...
class NRA_Engine:
    ...
    def process(self, particle: Particle):
        ...
        # --- Phase 3: Residue (廃棄) ---
            ...
            # 失敗: 厚みを減衰させる
            # ※注意: momentum は閾値調整用の内部変数であり、NRA-IDEの τ（厚み）そのものではない。
            # τ は物理的に常に正の値。momentum の負値は「閾値を厳格化する」効果のみを持つ。
