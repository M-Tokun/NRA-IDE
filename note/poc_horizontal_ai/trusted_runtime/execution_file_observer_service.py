"""One-request, separately launched execution file observer."""
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        signer = admit_file_signer(
            ...
            bundle_max_age=timedelta(seconds=args.trust_bundle_max_age_seconds),
        ...
        with PersistentNonceStore(
            ...
            response = process_signed_execution_file_observation_request(
                sys.stdin.read(),
                repository_root=Path(args.repository_root),
                observer_id=args.observer_id,
                nonce_store=nonce_store,
                admitted_signer=signer,
                request_max_age=timedelta(seconds=args.request_max_age_seconds),
            )
