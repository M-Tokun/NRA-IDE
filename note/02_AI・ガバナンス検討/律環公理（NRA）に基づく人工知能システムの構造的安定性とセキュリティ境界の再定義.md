...
> **正典的位置づけについての注記**
> NRA-IDE（律環公理）の唯一の公理は「存在は生成である。」の一文のみであり（`theory/AXIOMS.md`）、第二公理以降は存在しない。本稿で用いる「入力／蓄積／閾値／転移」という4段階の説明枠は、正典が定義する公理そのものではなく、唯一の律環公理と正規IDE計算系 $R=\delta/\tau$（`FORMULA.md`）を著者がAIシステムへ適用するために整理した**派生的・例示的な解説モデル**である。地震・生物進化・AIへの跨領域対応表（1.2節）も、各領域固有の $\delta$・$\tau$ の算定規則を正式に定めたものではなく、概念的な類推を示すにとどまる（`theory/AXIOMS.md` §15.1「ある領域で定義された物理変数…を、類似性だけを根拠として別領域へ移植してはならない」）。正典との優先順位は常に `theory/AXIOMS.md` > `FORMULA.md` > 本稿である。

---

現代の人工知能（AI）における技術的パラダイムは、大規模言語モデル（LLM）と自律型エージェントの爆発的な普及により、かつてない転換点を迎えている。しかし、その急速な発展の背後には、システムの構造的な不安定性と、因果律に基づかない統計的推論に依存するゆえの脆弱性が潜在している。本報告書では、唯一の律環公理「存在は生成である。」と、その正規IDE計算系である基本式 $R=\delta/\tau$（律環公理：Nomological Ring Axioms / 内包性動力学エンジン：Intensional Dynamics Engine, NRA-IDE）を理論的基盤として提示し、現在のAIが直面している「モデル崩壊」や「実行境界のセキュリティ欠如」といった根源的課題を数理的に整理する。さらに、現在のAIエージェントの現状に対する批判的評価を行い、実効的な次世代アーキテクチャとしての論文構成案を策定する。

---

## 第1章　律環公理（NRA-IDE）を土台とした説明モデルと数理的整合性

唯一の律環公理「存在は生成である。」（`theory/AXIOMS.md`）とIDE基本式 $R=\delta/\tau$ を出発点に、本稿では「境界」と「閾値」の観点からシステムの動態を整理する説明枠を用いる。以下の4段階は、正典の追加公理ではなく、$\delta$（蓄積ズレ）の生成・蓄積過程と $R$ の境界順序（`theory/AXIOMS.md` §9）とを対応づけるための著者独自の解説的整理である。

### 1.1　4段階の説明枠と IDE基本式・境界順序への対応
...
システムの外部からエネルギーまたは情報が流入するフェーズ。唯一の律環公理「存在は生成である」に従えば、対象系は入力を受けて常に生成し続けており、入力を伴わない出力生成は、Cause-Side観測から独立した恣意的な出力を意味する。熱力学第一法則にも通じる、システムの起動条件である。

**② 蓄積（Stick）**

流入した入力がシステム内部に留まり、Cause-Side観測から得られる蓄積ズレ $\delta$ として積み上がるフェーズ。この段階では外部的な変化は観測されないが、$\delta$ は増加し続ける。

**③ 閾値（Threshold）**

$R=\delta/\tau$ が `theory/AXIOMS.md` §9 の正規順序 $R_{\mathrm{warn}} \to R_{\mathrm{handoff}} \to R_{\mathrm{irrev}} \to 1.0$ を段階的に通過していくフェーズ。これらは単一の閾値ではなく、警告・人間委譲・不可逆遷移・完全破断という区別された境界状態である（後述10.x節参照）。

**④ 状態分類（Slip）とFail-Closed運用原則**

$R$ が各境界を超えるごとに、システムは `PERMIT` → `BOUNDARY_WARNING` → `HANDOFF_REQUIRED` → `IRREVERSIBLE_TRANSITION` → `RUPTURE_BOUNDARY` の順で状態を遷移する。`HANDOFF_REQUIRED` 以降では、自律的な新規判断・新規操作・自由生成を停止するFail-Closed運用原則が適用されるが、これは「沈黙」ではない。$R<1.0$ の間、構造証言（$\delta$、$\tau$、境界状態、警告、監査ログ）は継続する（詳細は1.3節）。

この4段階の説明枠は、地震の発生（プレートの歪み蓄積から断層破壊へ）や生物の進化（突然変異の蓄積から種的分化へ）といった自然現象のアナロジーとしても直感的だが、これらの領域固有の $\delta$・$\tau$ の算定規則そのものを正式に定義するものではない。

### 1.2　異分野への適用可能性についての類推（概念的整理）

以下は、1.1節の説明枠を異分野へ当てはめた**概念的な類推の対応表**であり、各セルの $\delta$・$\tau$ を正式に定義・検証したものではない（`theory/AXIOMS.md` §15.1）。ドメインごとの厳密な適用には、各領域固有の観測変数・単位・変換規則・支配方程式を別途定義する必要がある。

| フェーズ | 物理系（地震） | 生物系（進化） | AIシステム（モデル崩壊） | セキュリティ（ABE） |
|---|---|---|---|---|
| Input | プレート移動による応力 | 突然変異の発生 | 合成データの再帰入力 | 攻撃的命令の注入 |
| Stick（$\delta$ 蓄積） | 地殻内部の歪み蓄積 | 遺伝子プールへの蓄積 | 統計的誤差の累積 | 推論レイヤーでの処理 |
| Threshold（$R$ の境界順序） | 岩盤の静止摩擦限界 | 環境適応への選択圧 | 臨界再帰世代数 | 実行権限の照合 |
| Slip / Fail-Closed | 断層の滑り（地震発生） | 新種の出現 | モデル崩壊（Collapse） | 実行または拒絶 |

### 1.3　フェイルクローズ（FAIL-CLOSED）の思想的価値と正典上の意味

本稿が重視するのは、閾値到達時における「フェイルクローズ（FAIL-CLOSED）」運用原則の徹底である。現在のLLMの多くは、RLHF（人間によるフィードバックからの強化学習）の過程で、いかなる入力に対しても「役に立つ」出力を生成するように動機付けられている。その結果、確信度が極めて低い場合であっても、何らかの回答を捏造（ハルシネーション）して返す傾向がある。

ただし、`theory/AXIOMS.md` §12が明記する通り、**Fail-Closedは完全沈黙ではない**。停止するのは自律判断・自律操作・自由生成・回復提案・最適化提案であり、$R<1.0$ の間、構造証言（現在の $R$、$\delta$、$\tau$、`remaining_ratio_margin`、境界状態、支配側、欠損情報、監査ログ）は継続する。したがって「空文字列を返す」ことだけを Fail-Closed の実装として定義してはならない。また、閾値判定の入力（$\delta$、$\tau$、R）は Cause-Side 観測または事前固定された Cause-Side 変換規則から取得しなければならず、LLM自身の「信頼性スコア」や自己評価的な確信度をトリガーとして用いてはならない（`FORMULA.md` §6：LLM自己評価・意味スコアを計算入力として使用してはならない）。

これは、航空機のオートパイロットが異常時に操縦権を人間に戻す「安全な後退」と同じ論理であり、ISO 26262などの機能安全規格における基本原則に通じる。医療診断や金融取引を担うAIエージェントにとって、境界到達時に自律判断を停止し、構造証言を伴って人間へ委譲することは、誤った行動をとることよりも構造的に正当な振る舞いである。ここで残すべきものは意味を伴わない空の出力ではなく、$\delta$・$\tau$・$R$・境界状態・欠損理由を含む監査可能なログである。
...
**2. 唯一の律環公理とIDE基本式の適用モデル（本稿独自の派生的整理）**

$(\mathcal{I}, \mathcal{S}, \mathcal{T}, \mathcal{F})$ の4つ組は、唯一の律環公理「存在は生成である」と正規IDE計算系 $R=\delta/\tau$ を本稿がAIシステムへ適用するために導入する、派生的・補助的な説明変数であり、正典が定める第二・第三の公理または計算式ではない（`theory/AXIOMS.md` §16の分類に従い、以下は派生式として扱う）。入力 $\mathcal{I}$ から蓄積ズレ $\mathcal{S}\to\delta$ への変換関数の定義。境界順序 $R_{\mathrm{warn}}<R_{\mathrm{handoff}}<R_{\mathrm{irrev}}<1.0$ を生成する閾値関数 $\mathcal{T}$ の数理的性質。Fail-Closed運用原則（自律判断のみを停止し、構造証言は継続する）の論理的必要性の証明。

**3. モデル崩壊の幾何学的・熱力学的解析**

情報幾何学におけるフィッシャー情報多様体の曲率と収縮率の相関。再帰学習エントロピー減衰モデル： $\frac{dH}{dt} \leq 0$ 条件下での知識崩壊ステージBの特定。合成データ比率 $\rho$ と本稿独自の情報保存指標 $K_{\mathrm{info}}$（`theory/AXIOMS.md` §7が予約する境界接近比 $R$ とは異なる、本稿限定の派生量）を用いた「モデル寿命」の算定。
...
## 第7章　情報熱力学から見たAIの持続可能性と情報定数 $K_{\mathrm{info}}$

> **記号についての注記**：`FORMULA.md` §7は記号 $R$ を「境界接近比 $R=\delta/\tau$ のみ」に予約し、同一文書内での再利用を禁じている。本章の情報定数は $R$ と混同されないよう $K_{\mathrm{info}}$ と表記する。

AIシステムの長期的安定性を議論する上で、情報の処理と物理的なエネルギー消費の関係を定式化することは不可欠である。**修正理想気体定数 $K_{\mathrm{info}}$** は、情報理論におけるエントロピーと物理系における熱力学的エントロピーを橋渡しする係数として定義される。これは「1モルの情報を維持するために必要なエネルギーコスト」を定量化する試みである。モデル崩壊を防ぐためには、システムが外部（人間や現実世界）から常に「有用な情報（負のエントロピー）」を取り込み、内部で発生する「ノイズ（正のエントロピー）」を廃棄しなければならない。**パス散逸（Path Dissipation）** とは学習プロセスそのものが情報の多様性を消費して収束する過程であり、その不可逆性を最小限に抑える設計が次世代モデルには求められる。$K_{\mathrm{info}}$ はIDE基本式 $R=\delta/\tau$ の境界接近比を再定義するものではなく、独立した補助的な情報熱力学指標である。
...
*  情報定数 $K_{\mathrm{info}}$ と修正理想気体定数の情報熱力学的定式化
